...
function createCompleteService({ campus_name, service_name, service_time }) {
  cy.makeApiCall("POST", "/campuses", "AttendanceApi", [{ name: campus_name }]).then((res) => {
    const campus = res[0];
    cy.makeApiCall("POST", "/services", "AttendanceApi", [{ campusId: campus.id, name: service_name }]).then((services) => {
      const service = services[0];
      cy.makeApiCall("POST", "/servicetimes", "AttendanceApi", [{ serviceId: service.id, name: service_time }]);
    });
  });
}
